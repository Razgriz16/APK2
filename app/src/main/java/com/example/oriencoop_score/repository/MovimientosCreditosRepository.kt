...
class MovimientosCreditosRepository @Inject constructor(
    ...
    companion object {
        private const val TAG = "MovimientosCreditosRepository"
    }

    /**
     * Obtiene los movimientos de créditos para un usuario específico mediante la API.
     * Ejecuta la llamada de red en el hilo de IO y maneja la respuesta específica.
     *
     * @param  nroCuenta del usuario para filtrar los movimientos.
     * @return Un [Result] que contiene [Result.Success] con [MovimientosCreditos]
     *         o [Result.Error] con la [Throwable] correspondiente.
     */
    suspend fun getMovimientosCreditos(nroCuenta: Long): Result<ApiResponse<MovimientosCreditos>> {
        val functionName = "getMovimientosCreditos"
        Log.d(TAG, "$functionName: Iniciando llamada a la API para NroCuenta: $nroCuenta")
        return withContext(Dispatchers.IO) {
            try {
                val response = movimientosService.getMovimientosCreditos(nroCuenta)
