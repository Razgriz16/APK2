...
import com.example.oriencoop_score.di.MisMovimientosAhorro
...
@Singleton
class MovimientosAhorroRepository @Inject constructor(
    @MisMovimientosAhorro private val movimientosService: MovimientosService
) {

    // Define una etiqueta TAG constante para los logs de esta clase.
    companion object {
        private const val TAG = "MovimientosAhorroRepository"
    }

    /**
     * Obtiene los movimientos de ahorro para un usuario específico mediante la API.
     ...
     * @return Un [Result] que contiene [Result.Success] con [ApiResponse<MovimientosAhorro>]
     *         o [Result.Error] con la [Throwable] correspondiente.
     */
    suspend fun getMovimientosAhorro(numeroCuenta: Int, cantidad: Int, token: String): Result<ApiResponse<MovimientosAhorro>> {
        val functionName = "getMovimientosAhorro"
        val tokenBearer = "Bearer $token"
        Log.d(TAG, "$functionName: Iniciando llamada a la API para numeroCuenta: $numeroCuenta, cantidad: $cantidad")
        return withContext(Dispatchers.IO) {
            try {
                val response = movimientosService.getMovimientosAhorro(numeroCuenta, cantidad, tokenBearer)
